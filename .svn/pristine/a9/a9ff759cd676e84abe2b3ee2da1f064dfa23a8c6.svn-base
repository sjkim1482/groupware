...
<br>
<h1 style="text-align: center;">임시 저장함</h1><br>
...
<thead>
	...
	<c:forEach items="${appList}" var="app">
		<tr class="appDetailTempView" data-app_char="${app.app_char}">
			<td>${app.app_char}</td>
...
<div class="row">
	...
	<a href="javascript:pagingAppTempAjax(1,${pageVo.pageSize });">&lt;&lt;</a></li>
...
<%-- 			<a href="#">${i}</a></li> --%>
		...
		<c:otherwise>
			<li class="paginate_button " aria-controls="example" tabindex="0">
				<a href="javascript:pagingAppTempAjax(${i }, ${pageVo.pageSize});">${i }</a></li>
		</c:otherwise>
	</c:choose>
...
<li class="paginate_button next" aria-controls="example" tabindex="0" id="example_next">
	<a href="javascript:pagingAppTempAjax(${pagination}, ${pageVo.pageSize});">&gt;&gt;</a></li>
			</ul>
		</div>
	</div>
</div>	
