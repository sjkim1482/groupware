...
<!-- 			</form> -->
			...
			<table class="table table-inbox table-hover" style="width: 100%">
				<tbody>
					...
					<c:forEach items="${msgList}" var="msg" varStatus="status">
							...
							<tr class="delete_msg_no unread" data-msg_no="${msg.msg_no}">
								<td>
	...
	<a href="javascript:pagingDeleteMsgAjaxHtml(1,${pageVo.pageSize });">&lt;&lt;</a></li>
...
<%-- 			<a href="#">${i}</a></li> --%>
		...
		<c:otherwise>
			<li class="paginate_button " aria-controls="example" tabindex="0">
				<a href="javascript:pagingDeleteMsgAjaxHtml(${i }, ${pageVo.pageSize});">${i }</a></li>
		</c:otherwise>
	</c:choose>
...
<li class="paginate_button next" aria-controls="example" tabindex="0" id="example_next">
	<a href="javascript:pagingDeleteMsgAjaxHtml(${pagination}, ${pageVo.pageSize});">&gt;&gt;</a></li>
			</ul>
		</div>
	</div>
</div>	
