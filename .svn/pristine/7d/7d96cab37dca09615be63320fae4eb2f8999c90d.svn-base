...
public class PmsController {
	...
	public String workDetail(int work_no,int pms_no, Model model) {
		...
		/* 일감 첨부파일 리스트 */
		 List<Integer> workFileList = pmsService.workFileList(work_no);
		 logger.debug("workFileList:{}", workFileList );
		 List<AttFileVo> attFileList = new ArrayList<AttFileVo>();
			if(workFileList!=null) {
				for(int i = 0; i<workFileList.size(); i++){
					attFileList.add(attFileService.selectFile(workFileList.get(i)));
				}
			}
		/* 하위일감 리스트 */
	...
	public String registPms(int pms_no, int attend_emp_no, PmsVo pmsVo,  MultipartHttpServletRequest fileList, Model model) {
		...
		int insertCnt = 0;
		//일감 등록
		insertCnt = pmsService.registWork(pmsVo);
		if(insertCnt==1) {
			String filename = "";
			AttFileVo attFileVo = new AttFileVo();
			WorkFileVo workFileVo = new WorkFileVo();
			//해당 일감번호 조회
			workFileVo.setWork_no(pmsService.selectWork());
			work_no = workFileVo.getWork_no();
			int fileNo = 0;
			if(files!=null) {
				for(MultipartFile file : files) {
					if(!("".equals(file.getOriginalFilename()))) {
						try {
							...
							workFileVo.setFile_no(fileNo);
							logger.debug("workFileVo :{}", workFileVo);
							pmsService.registWorkFile(workFileVo);
		...
		if(pmsVo.getProg()==100) {
			pmsService.modifyWorkProg(pmsService.selectWork());
			logger.debug("work_no:{}", work_no);
		}
	...
	public String modifyPms(PmsVo pmsVo, int pms_no, int work_no, Model model) {
		...
		logger.debug("pmsVo222 : {}",pmsVo);
		 ...
		 if(pmsVo.getProg()==100) {
			 pmsService.modifyWorkProg(work_no);
		 }
		List<CommCdVo> commcdList = pmsService.selectComm_cd();
		List<CommCdVo> commcdPriList = pmsService.selectComm_cdPri();
