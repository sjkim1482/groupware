...
<%-- 	<h3>제목</h3>${boardPost.title}
	...
	<br><br> 
	<div id="mdBtn" style="float: right;">
	 <c:choose>
		...
		<c:when test="${S_USER.emp_no==0}">
			...
			<input type="button" class="btn btn-info" id="replyBtn" value="답글 등록">
			</c:if> 
	<h3>댓글</h3>
			...
			<c:choose>
				...
				<c:otherwise>
					...
					<c:if test="${S_USER.emp_no==comm.emp_no}">
						<input type="submit"  class="btn btn-primary" value="삭제"><br>
					</c:if>
					</form>
			</c:otherwise>
			</c:choose>
		...
		<form action="${cp}/board/insertComm" method="post">
			<textarea rows="5" cols="100" id="comm" name="cont"></textarea>
			<input type="hidden" name="post_no" value="${boardPost.post_no}">
			<input type="hidden" name="emp_no" value="${S_USER.emp_no}">
			<input  type="submit" id="commBtn" class="btn btn-primary" value="저장">
		</form>
