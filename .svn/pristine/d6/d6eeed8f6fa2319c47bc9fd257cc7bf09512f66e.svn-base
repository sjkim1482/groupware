		...
		<table border="1" style="width: 100%;">
			...
			<tr>
				<th
					...
					height="20"><strong>기안날짜</strong></th>
...
<!-- 			</tr> -->
		...
		<div style="float: right;">
			<input type="button" id="returnMsg" value="보관함으로 이동">
			<input type="button" id="realDeleteMsg" value="영구삭제">
		</div>
...
<script>
	$("#realDeleteMsg").on("click", function() {
		
		$("#moveMsgBoxFrm").attr("method","POST");
		$("#moveMsgBoxFrm").attr("action","${cp}/msg/deleteTempMsg");
		$("#moveMsgBoxFrm").submit();
	})
	$("#returnMsg").on("click", function() {
		
		$("#moveMsgBoxFrm").attr("method","POST");
		$("#moveMsgBoxFrm").attr("action","${cp}/msg/returnMsg");
		$("#moveMsgBoxFrm").submit();
	})
