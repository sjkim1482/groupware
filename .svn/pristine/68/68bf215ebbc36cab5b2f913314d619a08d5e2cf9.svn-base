				...
				<tbody>
					...
					<c:forEach items="${msgList}" var="msg" varStatus="status">
						<c:choose>
							...
							<tr class="unread rec_msg_no" data-msg_no="${msg.msg_no}">
								<td>
							...
							<c:otherwise>
							<tr class="rec_msg_no" data-msg_no="${msg.msg_no}">
								<td>
									<img id="pictureViewImg" style="width: 50px; height: 50px;"	src="${cp }/empController/profile?emp_id=${msg.emp_id}" />
								</td>
								<td>${msg.dept_nm}</td>
...
<div class="row">
	...
	<a href="javascript:pagingRecMsgAjaxHtml(1,${pageVo.pageSize });">&lt;&lt;</a></li>
...
<%-- 			<a href="#">${i}</a></li> --%>
		...
		<c:otherwise>
			<li class="paginate_button " aria-controls="example" tabindex="0">
				<a href="javascript:pagingRecMsgAjaxHtml(${i }, ${pageVo.pageSize});">${i }</a></li>
		</c:otherwise>
	</c:choose>
...
<li class="paginate_button next" aria-controls="example" tabindex="0" id="example_next">
	<a href="javascript:pagingRecMsgAjaxHtml(${pagination}, ${pageVo.pageSize});">&gt;&gt;</a></li>
			</ul>
		</div>
	</div>
</div>	
